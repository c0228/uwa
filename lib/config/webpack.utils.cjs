...
function webpackLibraryAliasConfiguration(modules) {
 ...
 modules.forEach((module) => {
  const webpackConfig = require(
   Path.resolve(
    __dirname + './../node_modules/' + module + '/webpack.config.cjs'
   )
  );
 ...
 fs.readFile(packageFile, (err, data) => {
  ...
  axios
   ...
   .then((response) => {
    ...
    if (versionType.toUpperCase() === 'MAJOR') {
     npmMajorVersion++;
    }
    let npmMinorVersion = parseInt(npmVersionSplit?.[1]);
    if (versionType.toUpperCase() === 'MINOR') {
     npmMinorVersion++;
    }
    let npmPatchVersion = parseInt(npmVersionSplit?.[2]);
    if (versionType.toUpperCase() === 'PATCH') {
     npmPatchVersion++;
    }
    setPackageVersion(
     packageJsonObj,
     npmMajorVersion + '.' + npmMinorVersion + '.' + npmPatchVersion
    );
   })
   .catch((error) => {
