...
export const getDaysOfWeekInMonth = (year, month, dayOfWeek, getData = 'NONE') => { // 2024:int, 10:int, 'Monday':String
  /*
   // Format the date as 'Sunday, 26 October 2024'
    {
        weekday: 'long',
        day: '2-digit',
        month: 'long',
  ...
  */
  ...
  const totalDays = new Date(year, month, 0).getDate(); // Get total number of days in the month
  const days = [];
  const today = new Date(); // Get today's date
  today.setHours(0, 0, 0, 0); 
  // Generate an array of all days in the month
  Array.from({ length: totalDays }, (_, i) => i + 1).forEach(day => {
    const date = new Date(year, month - 1, day);
    
     // Reset time to 00:00:00 for accurate date comparison
     date.setHours(0, 0, 0, 0);

    // Check if the date is the dayOfWeek we are looking for
    if (date.getDay() === daysOfWeekMap[dayOfWeek?.toLowerCase()]) {
      
      // Filter dates based on the getData parameter
      if (getData === 'AFTER' && date >= today) {
          days.push(defaultYYYYMMddFormat(date));
      } else if (getData === 'BEFORE' && date <= today) {
          days.push(defaultYYYYMMddFormat(date));
  ...
  return days; // Returns a list of Days of the specific dayOfWeek
};

const defaultYYYYMMddFormat=(date) =>{
