...
export const Range = ({ name, label, value, start, end, validation }) => {
 ...
 const validateRangeVal = async(value)=>{
  setValidationStatus(value);
   let result = { value: value, validationSuccess:[], errorMessage:'' }
   if (validation !== undefined) {
    result = await FormRangeValidate(validation, value);
   }
   RegisterToForm(result);
 };
 useEffect(()=>{
   validateRangeVal(value);
 },[value]);
  ...
  <>
   ...
   <div className="range-wrap">
    ...
    />
    {
     <output
      className="bubble"
      style={{ left: `calc(${position}% + (${8 - position * 0.15}px))` }}
     >
      {rangeValue}
     </output>
    }
     ...
     {(form?.[formName]?.submitted || rangeValue?.toString()?.length > 0) &&
      validationStatus?.errorMessage?.length > 0 && (
       <div align="right" className="form-feedback-validation-invalid" style={{ fontSize:'11px' }}>
        <InfoIcon /> {validationStatus?.errorMessage}
       </div>
      )}
    </div>
   </div>
