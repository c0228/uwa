...
export const Switch = ({ name, value, label, type, validation, layout }) => {
  ...
  const handleValidation = useCallback(async(switchValue) => {
   ...
   useEffect(() => {
    console.log("switched value", switchValue, "selectedSwitchValue ", selectedSwitchValue);
    handleValidation(switchValue);
  }, [switchValue]);
  ...
  const updateSwitchValues = (switchValue, e, id)=>{
    return switchValue?.map(item => item.id === id ? { ...item, checked: e.target.checked } : item)
  };
