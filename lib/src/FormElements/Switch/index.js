...
export const Switch = ({ name, value, label, type, validation, layout, onChange }) => {
 ...
 useEffect(()=>{
  const checkedItem = switchValue.find(item => item.checked);
  if(onChange!==undefined) { onChange(checkedItem); }
 },[switchValue]);
