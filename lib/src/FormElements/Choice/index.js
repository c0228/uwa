...
const handleValidation = useCallback(async(choiceValue)=>{
    // validation
    let result = { value: choiceValue.filter((v) => v?.checked === true) };
    if (validation !== undefined) {
       result = await FormSwitchValidate( type, validation, choiceValue.filter((v)=>(v?.checked===true)) );
    }
     // form Data
...
const validateOnSubmit =  (FormMode === FORM_SUBMITTED) || selectedChoiceValue?.length > 0;
...
{label && (<label className={((validateOnSubmit) ?
 ...
 <div style={{ color: (FormErrorMessage?.length > 0 && selectedChoiceValue?.length > 0) ? 'red' : '', fontWeight: 'bold' }}>
 {choiceValue.map((v,i)=>{
    return (<div key={i} className="form-check"  style={{ float: (layout==='horizontal'?'left':'none' )}}>
        <input className="form-check-input" type={type} style={{ width:'1.2em', height:'1.2em' }} name={name} 
        value={v?.value} id={name+"-"+v?.id} checked={v?.checked} 
        disabled={v?.disabled} onChange={(event)=>handleClick(type, event, v?.id)} />
        <label className="form-check-label" style={{ display:'flex', paddingTop:'1px', paddingRight:'15px', cursor:'pointer' }}
