...
export const FileDropperAndViewer = ({
 label,
 name,
 height,
 autoSubmit,
 ...
 const progressCalculator = (loaded, total) => {
  ...
  fileUploadBar = fileUploadBar.map((fillSize, index) => {
   ...
   const newFillSize = (
    parseFloat(fillSize) + parseFloat(loaded / totalFiles)
   ).toFixed(2);
 ...
 const onSubmit = async () => {
  ...
  try {
   ...
   //
   if (afterUploadSuccess) {
    afterUploadSuccess(JSON.stringify(fileUploaderDetails));
   }
  } catch (error) {
   console.error(error);
  }
 ...
 const validateOnSubmit =
  FormMode === FORM_SUBMITTED || fileNewUploadDetails?.length > 0;
     ...
     {fileUploaderDetails.map((file, index) => {
      ...
      return (
       <Row
        key={index}
        className="mbot15p"
        style={{ border: '1px solid #ccc', padding: '15px', margin: '5px' }}
       >
        ...
        <Col
         ...
         md={11}
         style={
          !isFileSubmitted ? { display: 'flex', alignItems: 'center' } : {}
         }
        >
         <div>
          <b>{fileName}</b>
         ...
         {isFileSubmitted &&
          (progressPercent < 100 ? (
           <>
            ...
            <div>
             {FormatBytes(progressSize)} of {FormatBytes(fileSize)} |{' '}
             {progressPercent}%
            </div>
           </>
          ) : (
           <div align="right" style={{ color: '#777' }}>
            <b>Uploaded File Successfully</b>
           </div>
          ))}
        </Col>
       </Row>
