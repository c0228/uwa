...
export const DateTimePicker = ({ type, label, name, value, minValue, maxValue, validation, onChange }) => {
  ...
  const RegisterToForm = async(result) =>{
...
console.log(formatDate(new Date()));   // Today's date in DD Month YYYY format
  ...
  */
   ...
   */
  const dateTimePickerInitialize = async(value, t) =>{
    ...
    let result ={ value: newValue, validationSuccess: [], errorMessage: [] };
    if (validation !== undefined) {
      result = await FormInputValidate(validation, newValue);
    }
     // Min/Max validation
    if (newValue) {
  ...
  useEffect(() => {
    dateTimePickerInitialize(value,t);
  }, [value, t]);
