...
export const Password = ({
 ...
 const pwdNoMatch =
  Array.isArray(errMessage) && errMessage.includes('PASSWORD_NOT_MATCH');
 const charValidated =
  Array.isArray(errMessage) &&
  !errMessage.includes('MINLENGTH_FAILED') &&
  !errMessage.includes('MAXLENGTH_FAILED');
 const lowerCaseValidation =
  Array.isArray(errMessage) && !errMessage.includes('LOWERCASE_FAILED');
 const upperCaseValidation =
  Array.isArray(errMessage) && !errMessage.includes('UPPERCASE_FAILED');
 const numberValidation =
  Array.isArray(errMessage) && !errMessage.includes('NUMBER_FAILED');
 const symbolValidation =
  Array.isArray(errMessage) && !errMessage.includes('SYMBOL_FAILED');
 ...
 const validateOnSubmit =
  FormMode === FORM_SUBMITTED || passwordValue?.length > 0;
   ...
   {validation && validateOnSubmit && isErrorMessageExist && (
    <div style={{ fontSize: '12px', fontWeight: 'bold', padding: '10px' }}>
     <div style={{ padding: '3px', color: charValidated ? 'green' : 'red' }}>
      <Icon
       type="FontAwesome"
       name={charValidated ? 'fa-check-circle' : 'fa-times-circle'}
       size="14"
      />
      <span style={{ paddingLeft: '8px' }}>
       It should be in between 8-12 Characters
      </span>
     </div>
     <div
      style={{ padding: '3px', color: lowerCaseValidation ? 'green' : 'red' }}
     >
      <Icon
       type="FontAwesome"
       name={lowerCaseValidation ? 'fa-check-circle' : 'fa-times-circle'}
       size="14"
      />
      <span style={{ paddingLeft: '8px' }}>It should contain a Lowercase</span>
     </div>
     <div
      style={{ padding: '3px', color: upperCaseValidation ? 'green' : 'red' }}
     >
      <Icon
       type="FontAwesome"
       name={upperCaseValidation ? 'fa-check-circle' : 'fa-times-circle'}
       size="14"
      />
     ...
     <div style={{ padding: '3px', color: numberValidation ? 'green' : 'red' }}>
      <Icon
       type="FontAwesome"
       name={numberValidation ? 'fa-check-circle' : 'fa-times-circle'}
       size="14"
      />
     ...
     <div style={{ padding: '3px', color: symbolValidation ? 'green' : 'red' }}>
      <Icon
       type="FontAwesome"
       name={symbolValidation ? 'fa-check-circle' : 'fa-times-circle'}
       size="14"
      />
