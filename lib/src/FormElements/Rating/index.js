...
export const Rating = ({ name, size, colors, position, value, validation })=>{
 ...
 const [ratingValue, setRatingValue] = useState( (value!==undefined)?(value):(0) );
 ...
 const handleValidation = useCallback(async(ratingValue)=>{
...
const isStatic = (position!==undefined && position?.toLowerCase()==='static');
 return (<>
    {Array.from({ length: size }).map((_, index)=>{
        const data = index+1;
        return (<Icon key={index} type="FontAwesome" name={data<=ratingValue?"fa-star":"fa-star-o"} 
        size={20} style={{ marginRight:'5px', color: ColorManager(ratingValue) }} 
        onMouseEnter={()=>{ 
            if(!isStatic){ 
                setRatingValue(data);
                setClicked(false); 
