...
import { FormInputValidate, getForm, FORM_SUBMITTED, FORM_RESET, getResponsiveStyle } from 'e-ui-react';
...
export const Select = ({
 ...
 useEffect(async()=>{
  setSelectValue(initialValue);
   let result = { value: initialValue, validationSuccess:[], errorMessage:'' }
   if (validation !== undefined) {
    result = await FormInputValidate(validation, initialValue);
   }
 ...
 let sWidth = width === undefined ? '100%' : width;
 let sfontSize = fontSize === undefined ? '12' : fontSize;
 ...
 if (mediaQuery?.matches) {
  selectStyles = {
   ...selectStyles,
   width: getResponsiveStyle(sWidth),
   fontSize: getResponsiveStyle(sfontSize),
  };
 } else {
  selectStyles = {
   ...selectStyles,
   ...
   >
    <select
     className={
      validateOnSubmit
       ? isErrorMessageExist > 0
         ? 'form-select form-control-validation-invalid ' + sClassName
