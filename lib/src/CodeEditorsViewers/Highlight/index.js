...
export const Highlight = (props) => { // props.withBlink
 //  lines={['1','3', '5T8']}
 // (props.content, props.lang, props.lines)
 const displayCode = RequireCode( props.content, props.lines );
 const highlightCode = hljs.highlight(displayCode, {language: (props.lang!==undefined)?props.lang:'html' }).value;
 ...
 const addBlinkFeature = () => {
  ...
  if (blinkIndex <= blinkWord.length) {
    ...
    setCurrentBlinkWord(currentBlinkWord.substring(0, currentBlinkWord.length-1));
  }
};

useEffect(() => {
  if(props.withBlink){ setTimeout(() => addBlinkFeature(), 100); }
}, [blinkIndex]);

useEffect(()=>console.log('currentBlinkWord',(currentBlinkWord)),[currentBlinkWord]);
 ...
 useEffect(()=>{
  ...
  let hglHtmlContent = (props?.withBlink===true)?HtmlStringToReactJSXConverter(currentBlinkWord):
  ...
  const CopyClip = ({ icon, title })=>{
    const handleClick = ()=>{
      ...
      setTimeout(()=>{ setHoverContent(initialHoverContent); },2000);
      navigator.clipboard.writeText( displayCode );
    };
    return (<div style={{ position: 'relative' }}>
      <div align="right" style={{ position:'absolute', zIndex:'1', right:'8px', top:'12px' }} onClick={()=>handleClick()}>
        <span className="bs-hglCopy-icon">
  ...
  return (<>
    ...
    <pre style={{ borderRadius:'5px' }}><code className={"lang-"+props.lang} {...props}>
    {hglHtmlContent}
    </code></pre>
  </>);
