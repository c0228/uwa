...
export const TreeView = ({
  ...
  if (selectedFile !== undefined) {
   ...
   childElements?.forEach(function (childId) {
    ...
    if (childId === selectedFile) {
     document
      .getElementById(id)
      ?.querySelector('#' + selectFile)
      ?.classList.add('bold');
     document
      .getElementById(parentId)
      ?.querySelector('#' + childId)
      ?.classList.remove('treeViewerComponent-hide');
     document
      .getElementById(parentId)
      ?.querySelector('#' + childId)
      ?.classList.add('treeViewerComponent-show');
    } else {
     document
      .getElementById(id)
      ?.querySelector('#' + treeChildId)
      ?.classList.remove('bold');
     document
      .getElementById(parentId)
      ?.querySelector('#' + childId)
      ?.classList.remove('treeViewerComponent-show');
     document
      .getElementById(parentId)
      ?.querySelector('#' + childId)
      ?.classList.add('treeViewerComponent-hide');
    }
   });
  }
 };
 ...
 const View = (files) => {
  ...
  const handleCheckBox = (targetId) => {
   const updateFiles = (files, status) =>
    files?.map((file) => {
  ...
  const GetFileFormat = (fileName, settings) => {
   ...
   let color =
    settings?.[fileExtension] !== undefined
  ...
  return (
   <ul id={id} className="tree">
    {files?.map((file, index) => {
     if (file?.type === 'folder') {
      return (
       <li key={index} className="treeContent" id={file?.id}>
        <div style={{ padding: '2px', cursor: 'pointer' }}>
         <i
          className={file?.open ? 'fa fa-caret-down' : 'fa fa-caret-right'}
         ...
         {checked && (
          <>
           ...
           <input
            ...
            onClick={() => handleCheckBox(file?.id)}
         ...
         >
          <i
           className={file?.open ? 'fa fa-folder-open' : 'fa fa-folder'}
           style={{
            color:
             settings?.['folder'] !== undefined
              ? settings?.['folder']
              : settings?.['default'] !== undefined
                ? settings?.['default']
                : '#000',
        ...
        <div className={file?.open ? 'open' : 'close'}>
         {' '}
         {View(file?.children)}
        </div>
       </li>
      ...
      return (
       <li key={index} className="treeContent">
        <div style={{ padding: '2px', marginLeft: '9px' }}>
         ...
         <span
          ...
          onClick={() => {
           onFilesClick(file?.id);
          }}
         >
          {' '}
          {GetFileFormat(file?.name, settings)}&nbsp; {file?.name}
         </span>
        </div>
       </li>
      );
