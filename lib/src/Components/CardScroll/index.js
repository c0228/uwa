...
export const CardScroll = (props) => {
 ...
 let bgColor =
  props?.backgroundColor === undefined ? 'none' : props?.backgroundColor;
 let padding = props?.padding === undefined ? '0px' : props?.padding + 'px';
 let borderRadius =
  props?.borderRadius === undefined ? '8px' : props?.borderRadius + 'px';
 ...
 return (
  <div className={'bs-card-scroll' + classes} style={styles}>
   {props?.children}
  </div>
 );
};
