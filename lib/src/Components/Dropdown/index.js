...
export const Dropdown = ({ width, label, name, type, placeholder, menu, layout, value, validation, onChange,
 valueholder, container
  ...
  const initialValue = () => {
    if (type === "multiple") {
      return value === undefined ? [] : ( Array.isArray(value)?value:(value?.split(",")) );
    } else {
  ...
  const SearchData = (filterData) => {
    ...
    const filteredMenu = menu?.reduce((result, item) => {
        ...
        if (header?.toLowerCase()?.includes(filterData?.toLowerCase())) {
          ...
          if (filteredOptions?.length > 0) { result?.push({ header, options: filteredOptions }); }
        } else {
          const filteredOptions = options?.filter(option =>
            option?.label?.toLowerCase()?.includes(filterData?.toLowerCase())
          );
    ...
    return menu?.map((m, index) => {
      ...
      return (
        <div key={key}>
            ...
            <li>
              <h5 className="dropdown-header">
                {htmlToReactParser.parse(
                  m.header.replace(new RegExp(searchValue, "gi"), (str) => `<b>${str}</b>`)
                )}
              </h5>
            </li>
          )}
          {m?.label && (
            <li
              className={`dropdown-item curpoint ${type === 'multiple' && 
              Array.isArray(dropDownValue) && dropDownValue.includes(m.label) ? "dropdown-test" : ""}`}
              onClick={() => {
                if (type === "multiple") {
            ...
            >
              {type === 'multiple' && Array.isArray(dropDownValue) && dropDownValue.includes(m.label) ? (
                <>
                  {htmlToReactParser.parse(m.label.replace(new RegExp(searchValue, "gi"), (str) => `<b>${str}</b>`))}
      ...
      >
        <div className="dropdown" style={{ width: width }}>
          <button
            ...
            style={{ width: '100%', textAlign: 'left', border:'1px solid #ccc', margin:'0px' }}
          ...
          <ul className={`dropdown-menu ${status ? "show" : ""}${layout === "top" ? " dropup-menu" : ""}`} data-popper-placement="top-start">
            <div className="input-group mb-0">
              <input className="form-control" placeholder={container?.searchLabel} onChange={(event) => {
                setSearchValue(event.target.value);
              }} />
              <button type="button" className="btn btn-secondary">
                <Icon type="FontAwesome" name="fa-search" size="12" color={Colors.light} />
              </button>
            </div>
            {type==='multiple' &&
            (<div className="container-fluid">
              <div className="row">
                <div className="col-xl-6" style={{ paddingRight: '0px' }}>
                  <button type="button" className="btn btn-outline-primary" style={{ width: '100%', borderRight: '0px', borderTopRightRadius: '0px', borderBottomRightRadius: '0px' }} onClick={handleSelectAll}>Select All</button>
                </div>
                <div className="col-xl-6" style={{ paddingLeft: '0px' }}>
                  <button type="button" className="btn btn-outline-primary" style={{ width: '100%', borderTopLeftRadius: '0px', borderBottomLeftRadius: '0px' }} onClick={handleDeselectAll}>Deselect All</button>
                </div>
              </div>
            </div>)}
            <div className="dropdown-scroll scroll" style={{ maxHeight: (container?.height!==undefined)?container?.height:'100px'}}>
