...
export const Menu = React.memo( ({ type, layout, layoutStyle, align, list, justify, minValues, settings }) => {
  ...
  const Tab = ({ justify, list, minValues }) => {
    ...
    const [dropdownList, setDropdownList] = useState(list.slice(minValues, list.length));
    ...
    displayTabs?.map((l, i) => {
      tabList.push(<li key={i} className="nav-item">
        <a className={l.id === activeTab ? "nav-link active" : "nav-link"} data-bs-toggle="tab"
          href="#" onClick={() => setActiveTab(l.id)}><b>{l.label}</b></a>
      </li>);
      tabContent.push(<div key={i} className={l.id === activeTab ? "tab-pane tab-active" : "tab-pane fade"} id={l.id} 
      style={tabContentCss}>{l.component}</div>)
    });
    if(minValues!==undefined){
      tabList.push(<li className="nav-item" onBlur={(e) => {
        if (!e.currentTarget.contains(e.relatedTarget)) { setShowDropdown(false); }
      }}>
        <a className="nav-link" data-bs-toggle="tab"
          href="#">
            <Icon type="FontAwesome" name="fa-plus-circle" size="12" color="primary" onClick={()=>setShowDropdown(!showDropdown)}/>
            <ul className={(showDropdown)?"dropdown-menu open":"dropdown-menu"} style={{ zIndex:1000 }}>
              {dropdownList?.map((d, i)=>{
                return (<li key={i} onClick={()=>onDropdownMenuSelect(d)}><a className="dropdown-item" href="#">{d?.label}</a></li>);
              })}
            </ul>
        </a>
      </li>);
  ...
  const Pills = React.memo( ({ justify, list })=>{
    ...
    list.map((l, i) => {
      tabList.push(<li key={i} className="nav-item">
        <a id={l.id} className={l.id === activePill ? "nav-link active" : "nav-link"} data-bs-toggle="tab"
          href={"#"+l.id} onClick={() =>{ 
            setActivePill(l.id);
          }}><b>{l.label}</b></a>
      </li>);
      tabContent.push(<div key={i} className={l.id === activePill ? "tab-pane pill-active" : "tab-pane fade"} 
        id={l.id}>{l.component}</div>)
    })
    return (<>
    {(layout === 'vertical')?(<>
      <ContainerFluid >
        <Row>
          <Col md={layoutStyle?.menu?.md}
