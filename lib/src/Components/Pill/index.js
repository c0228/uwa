...
export const Pill = ({ mode, layout, align, sequence, menulinks, activeId, colorConfig }) => {
 ...
 const Menu = ()=>{
  if(mode === undefined || mode === 'horizontal'){
    return ( <ul className={("nav nav-pills " + alignment).trim()}  style={{ flexDirection: 'row', marginBottom:'20px' }}>
     <PillContent />
    </ul>);
  } else {
      return (<div align={align}>
          <ul className={"nav nav-pills flex-column "} style={{ marginBottom:'20px' }}>
            <PillContent />
          </ul>
        </div>);
  }
