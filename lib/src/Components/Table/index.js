...
import { Icon, ReactJSXToOutputViewer, ReactJSXToHtmlStringConverter, HtmlStringToReactJSXConverter, SortJSONArray } from 'e-ui-react';
import './index.css';
...
export const Table = ({ title, height, columnDesc, data, dataSettings }) => {
  ...
  if (isDataSequence) {
    colDesc?.unshift({ columnName: "#", id: "#", width: '25px', sortBy: "asc" });
  }
  ...
  const updateColumnSorting = (column) => {
   ...
   // setSearchInputs({ ...searchInputs, [column.id]: '' });
    const sortedData = SortJSONArray(tableDataSearch, updatedSortColumns?.id, updatedSortColumns?.sortBy);
    ...
    Object.entries(searchInputs).forEach(([column, searchValue]) => {
      if (searchValue && searchValue.length > 0) {
        filteredData = filteredData.filter((el) => {
          ...
          return columnData?.toString().toLowerCase().includes(searchValue.toLowerCase());
        });
      }
    });
    ...
    if (searchedText && searchedText.length > 0) {
      filteredData = filteredData.filter((el) => {
        return Object.values(el).some((value) => {
          ...
          return columnData?.toString().toLowerCase().includes(searchedText.toLowerCase());
        });
      });
    }
    ...
    <>
      <div className="row mbot10p">
        <div className="col-md-8" style={{ display: 'flex', alignItems: 'center' }}>
          {title && (<label style={{ fontSize: '14px', fontWeight: '400' }}>{title}</label>)}
        </div>
      ...
      <div className="row">
        <div className="col-md-12">
          <div className="table-responsive scroll" style={{ height: height, overflowX: 'auto' }}>
            <table className="table table-striped table-hover" style={{ marginBottom: '0px', backgroundColor: '#fff' }}>
              <thead>
                <tr align="center">
                  {colDesc?.map((col, index) => (
                    <th key={index} align={col?.titleAlign} style={{ position: 'relative', padding: '10px' }}
                        ...
                        <span className="float-end">
                          {sortColumns.id === col.id && (
                            <span style={{ paddingLeft:'3px', paddingRight:'3px' }}>
                                <Icon type="FontAwesome" name={"fa-sort-amount-" + sortColumns.sortBy} />
                            </span>
                          )}
                        </span>
                      </div>
                ...
                {tableDataSearch?.map((d, i) => (
                  <tr align="center" key={i}>
                    {colDesc?.map((col, index) => {
                      ...
                      if (searchedText && searchedText.length > 0) {
                        const searchIndex = cellContent.toLowerCase().indexOf(searchedText.toLowerCase());
                        if (searchIndex !== -1) {
                          const beforeText = cellContent.substring(0, searchIndex);
                          const highlightedText = cellContent.substring(searchIndex, searchIndex + searchedText.length);
                          const afterText = cellContent.substring(searchIndex + searchedText.length);
                      ...
                      const highlighedContentOutput = ReactJSXToOutputViewer( highlightedContent );
                      const searchInput = searchInputs[col.id];
                      if (searchInput && searchInput.length > 0) {
                        const searchIndex = highlighedContentOutput?.toLowerCase().indexOf(searchInput.toLowerCase());
                        if (searchIndex !== -1) {
                          const beforeText = highlighedContentOutput?.substring(0, searchIndex);
                          const highlightedText = highlighedContentOutput?.substring(searchIndex, searchIndex + searchInput.length);
                          const afterText = highlighedContentOutput?.substring(searchIndex + searchInput.length);
