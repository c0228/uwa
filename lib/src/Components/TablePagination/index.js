 ...
 *  
 */
export const TablePagination = ({ header, api, pageSize = 10, mode = "pagination" }) => {
  ...
  useEffect(()=>{ console.log("Table pagination: ", data)},[data]);
  // Fetch data from API
  const fetchData = async () => {
    ...
    try {
      ...
      console.log('Updated data:', mode === "scrollable" ? [...data, ...newData] : newData);
  ...
  return (
    <div>
      ...
      <div className="table-responsive">  
            ...
            {(data?.length===0) && (<div style={{ borderLeft:'1px solid #ccc', 
              borderRight:'1px solid #ccc', borderBottom:'1px solid #ccc',
              textAlign:'center', padding:'5px' }}>No Data Available</div>)}
      </div>
      ...
      {mode === "pagination" && (
        <div className="mtop15p">
          {Array.from({ length: totalPages }, (_, index) => index + 1).map(
            (page) => (<Button key={page} type="outline-dark" size={11} 
              onClick={() => handlePageChange(page)} style={{ margin:'5px' }}
                disabled={page === currentPage}><b>{page}</b></Button>)
          )}
        </div>
      )}
    </div>
