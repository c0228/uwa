...
export const Nav = ({ mode, layout, align, sequence, menulinks, activeId, colorConfig }) => {
 ...
 const NavContent = ()=>{
  return (menulinks?.map((data, index) => {
    let activeStyle = { cursor:'pointer', padding: '6px 10px', color: activeShowColor, marginBottom:'15px',
        background: activeBgShowColor, textDecoration: 'none', borderBottom: '2px solid ' + activeShowColor, 
        fontWeight: 'bold' };
    let defaultStyle = { cursor:'pointer', padding: '6px 10px', textDecoration: 'none',
        background: defaultBgShowColor, color: defaultShowColor, fontWeight: 'bold' };
    return (<li 
      className={(data?.disabled && (data?.disabled === true)) ? "nav-item pad5p disabled" : "nav-item pad5p"} 
      key={index} 
      onClick={()=>{
        setActId(data?.id);
        ...
        {typeof data?.label === 'string'?(<>
          <span style={((actId === undefined && index === 0) || (actId === data?.id)) ? 
 ...
 const Menu = ()=>{
  if(mode === undefined || mode === 'horizontal'){
    return ( <ul className={("navbar-nav " + alignment).trim()}  style={{ flexDirection: 'row' }}>
     <NavContent />
    </ul>);
  } else {
