...
import { Row, Col, UrlParams, Colors, pxTovh } from "e-ui-react";
import './index.css';
...
export const Nav = ({ mode, layout, align, sequence, menulinks, activeId, colorConfig }) => {
 ...
 const NavContent = ()=>{
  ...
  return (menulinks?.map((data, index) => {
    let activeStyle = { cursor:'pointer', color: activeShowColor, textDecoration: 'none', fontSize:pxTovh(12), 
              background: activeBgShowColor, fontWeight: 'bold' };
    if(mediaQuery?.matches) { 
        activeStyle = {...activeStyle, padding:pxTovh(5), borderBottom: pxTovh(2)+' solid '+activeShowColor }; 
    } else { activeStyle = {...activeStyle, paddingBottom:'5px', borderBottom: '2px solid ' + activeShowColor }; }
    
    let defaultStyle = { cursor:'pointer', textDecoration: 'none', background: defaultBgShowColor, 
        color: defaultShowColor, fontWeight: 'bold', fontSize:pxTovh(12) };
    if(mediaQuery?.matches) { 
        defaultStyle = {...defaultStyle, padding:pxTovh(5) }; 
    } else  { defaultStyle = {...defaultStyle, paddingBottom:'5px' }; }

    return (<li 
      className={(data?.disabled && (data?.disabled === true)) ? "nav-item mbot15p mrgt15p disabled" : "nav-item mbot15p mrgt15p"} 
      key={index} 
      onClick={()=>{
        setActId(data?.id);
 ...
 const Menu = ()=>{
  if(mode === undefined || mode === 'horizontal'){
    return (<div style={{ marginTop: pxTovh(8) }}>
