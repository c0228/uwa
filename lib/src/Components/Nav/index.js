...
export const Nav = ({ mode, layout, align, sequence, menulinks, activeId, colorConfig }) => {
 ...
 const NavContent = ()=>{
  ...
  return (menulinks?.map((data, index) => {
    let activeStyle = { cursor:'pointer', color: activeShowColor, textDecoration: 'none', fontSize:getResponsiveStyle(12), 
              background: activeBgShowColor, fontWeight: 'bold' };
    if(mediaQuery?.matches) { 
        activeStyle = {...activeStyle, padding:getResponsiveStyle(5), borderBottom: getResponsiveStyle(2)+' solid '+activeShowColor }; 
    ...
    let defaultStyle = { cursor:'pointer', textDecoration: 'none', background: defaultBgShowColor, 
        color: defaultShowColor, fontWeight: 'bold', fontSize:getResponsiveStyle(12) };
    if(mediaQuery?.matches) { 
 ...
 const Menu = ()=>{
  if(mode === undefined || mode === 'horizontal'){
    return (<div style={{ marginTop: getResponsiveStyle(8) }}>
