...
export const Tab = ({ justify, menulinks, minValues, settings }) => {
    ...
    const [dropdownList, setDropdownList] = useState(menulinks?.slice(minValues, menulinks?.length) || []);
    ...
    displayTabs?.map((l, i) => {
      tabList.push(<li key={i} className="nav-item">
        <div className={l.id === activeTab ? "nav-link active" : "nav-link"} data-bs-toggle="tab" 
            style={{ cursor:'pointer' }}
          onClick={() =>{
            setActiveTab(l?.id);
            if(l?.onClick!==undefined){ l?.onClick(); }
            if(l?.url!==undefined) { window.location.replace(l?.url); }
          }}><b>{l?.label}</b></div>
      </li>);
    ...
    if(minValues!==undefined){
      tabList.push(<li className="nav-item" onBlur={(e) => {
        ...
        <div data-bs-toggle="tab" style={{ color:'#0d6efd', padding:'8px 16px' }}>
            <Icon type="FontAwesome" name="fa-plus-circle" size="12" color="primary" style={{ cursor:'pointer' }}
            onClick={()=>setShowDropdown(!showDropdown)}/>
            <ul className={(showDropdown)?"dropdown-menu open":"dropdown-menu"} style={{ zIndex:1000 }}>
              {dropdownList?.map((d, i)=>{
                return (<li key={i} onClick={()=>onDropdownMenuSelect(d)}>
                    <div className="dropdown-item" style={{ cursor:'pointer' }}>{d?.label}</div></li>);
              })}
            </ul>
        </div>
      </li>);
