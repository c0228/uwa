...
*/
export const SideWrapperHeader = ({ isOpen, barTheme, colorConfig,  title, children }) => {
  const { toggle, setToggle } = useContext(SideWrapperContext);
  ...
  return (
    <>
      <nav className="navbar navbar-expand-lg" 
        style={{ backgroundColor: colorConfig?.backgroundColor || '#fbfbfb', 
     ...
     <div className="row">
         ...
         <a className="navbar-brand" href="#" 
             onClick={()=>{ setToggle(!toggle);setButtonToggle(!buttonToggle); }}>
               <Button type={theme} style={{ color: colorConfig?.color || '#000',
                border:'1px solid '+colorConfig?.color || '#000', background:'none' }}>
                  <Icon type="FontAwesome" name={(buttonToggle)?"fa-bars":"fa-close"} size="18" />
                </Button>
          </a>
